...
def construct_motif(txt_name, times, type, alpha_value):
    ...
    # 下面将上面由参数alpha_value得到的两个矩阵进行非线性融合
    result_B.data = np.array(map(lambda x: 0 if x == 0 else np.power(x, alpha_value), result_B.data), dtype=np.float64)

    result_C = motif_times(adjacency_matrix, times, type)
    result_C.data = np.array(map(lambda x: 0 if x == 0 else np.power(x, 1-alpha_value), result_C.data), dtype=np.float64)
    result_temp1 = result_B.multiply(result_C)
    result_D = result_temp1.tocsr()
    print maxnum
