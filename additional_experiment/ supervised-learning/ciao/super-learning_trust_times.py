...
if __name__ == "__main__":
    ...
    txt_name = 'average_help_ciao.txt'
    dict_help = construct_trust(txt_name)
    output_txt = 'slearning_Epinions/slearning_ciao_M1.txt'
    output = open(output_txt, 'w')
    for mmmm in range(10):
        for nn in range(len(alpha)):
            a, entry_unique = construct_motif('trust_id_ciao.txt', 1, alpha[nn], 'M1')
            # M = graphMove_new(a)
            maxnum1 = len(entry_unique)
            entry_list = np.linspace(0, maxnum1 - 1, num=maxnum1, dtype=np.int)
            ...
            #dict_t1 = construct_close('slearning_Epinions/close_ciao_M1_0%d.txt' % nn)
            #dict_t2 = construct_close('slearning_Epinions/between_ciao_M1_0%d.txt' % nn)
            ...
            for mm in range(1500):
                ...
                if not (dict_help.get(entry_unique[num_element])):
                    continue
                else:
                    h_index = dict_help[entry_unique[num_element]]
